...
namespace TribalRaiders_Code {
	...
	[HarmonyPatch(typeof(PawnBioAndNameGenerator), "GetBackstoryCategoryFiltersFor", new Type[] { typeof(Pawn), typeof(FactionDef) })]
	public static class PawnBioAndNameGenerator_GetBackstoryCategoriesFor {
		public static void Postfix(Pawn pawn, FactionDef faction, ref List<BackstoryCategoryFilter> __result) {
			if (faction.defName == "TribalRaiders") {
				List<BackstoryCategoryFilter> strs = new List<BackstoryCategoryFilter>();
                var backstoryCategoryFilter = new BackstoryCategoryFilter();
                backstoryCategoryFilter.categories = new List<string> { "TribalRaider" };
		...
		public static bool Prefix() {
			if (Controller.Settings.tribalPlanet.Equals(false)) {
				...
				foreach (FactionDef allDef in DefDatabase<FactionDef>.AllDefs) {
					if (allDef.defName == "OutlanderCivil" || allDef.defName == "OutlanderRough" || allDef.defName == "Pirate" || allDef.defName == "Empire") { continue; }
					else {
				...
				while (num < 5) {
					...
					if (factionDef.defName == "OutlanderCivil" || factionDef.defName == "OutlanderRough" || factionDef.defName == "Pirate" || factionDef.defName == "Empire") { continue; }
					else {
